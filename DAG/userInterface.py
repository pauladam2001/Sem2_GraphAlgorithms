...
class UI:
    ...
    def print_graph(self):
        for index in self._graph.dictionaryOUT:
            for j in self._graph._dictionaryOUT[index]:
                print('Source vertex: ' + str(index) + ', Destination vertex: ' + str(j) + ', Cost: ' + str(
                    self._graph._dictionaryCost[(index, j)]))

    def DAG_functionality(self):
        self._graph.init_durations()
        self._graph.init_prerequisites()
        self._graph.create_corresponding_graph()

        sorted = self._graph.TopSortPredecessorCounting()
        if sorted == None:
        ...
        earliestScheduling = self._graph.earliest_scheduling(sorted)
        projectDuration = 0
        for vertex in sorted:
            for find in earliestScheduling:
                if vertex == find:
                    if earliestScheduling[vertex][1] > projectDuration:
                        projectDuration = earliestScheduling[vertex][1]
                    print(str(vertex) + ': ' + str(earliestScheduling[vertex][0]) + ' ' + str(earliestScheduling[vertex][1]))
        print('\n')

        print('The latest scheduling:')
        latestScheduling = self._graph.latest_scheduling(sorted, projectDuration)
        for vertex in sorted:
            for find in latestScheduling:
                if vertex == find:
                    print(str(vertex) + ': ' + str(latestScheduling[vertex][0]) + ' ' + str(latestScheduling[vertex][1]))
        print('\n')

        print('\nThe total time of the project is ' + str(projectDuration))
        ...
        for vertex in sorted:
            for find in earliestScheduling:
                if vertex == find:
                    if earliestScheduling[vertex][0] == latestScheduling[vertex][0] and earliestScheduling[vertex][1] == latestScheduling[vertex][1]:
                        print(vertex)
    ...
    def print_menu(self):
        ...
        print('19. Verify if the corresponding graph is a DAG performing a topological sorting of the activities using the algorithm based on predecessor counters;')
        print('    Print the earliest and the latest starting time for each activity and the total time of the project;')
        print('    Print the critical activities.')
        print('\n\n')
    ...
    def start_program(self):
        ...
        menuItems = {'0': self.exit_ui, '15': self.read_graph_ui, '18': self.print_graph,
                     '19': self.DAG_functionality}

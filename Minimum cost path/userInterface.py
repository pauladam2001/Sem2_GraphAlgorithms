...
class UI:
    ...
    def minimum_cost_path(self):
        self._graph.init_visited()
        sourceVertex = int(input('\nSource vertex: '))
        targetVertex = int(input('Destination vertex: '))
        minCostPath = self._graph.minimum_cost_path(sourceVertex, targetVertex)
        if minCostPath != float('inf'):
            print('\nThe minimum cost path between the given vertices is ' + str(minCostPath))
        else:
            print('\nThere is no minimum cost path between the given vertices')
